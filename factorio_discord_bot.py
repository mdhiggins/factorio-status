...
async def get_factorio_status():
    try:
        logging.info(f"Attempting to connect to Factorio server at {FACTORIO_SERVER_IP}:{FACTORIO_RCON_PORT}")
        
        with MCRcon(FACTORIO_SERVER_IP, FACTORIO_SERVER_PASSWORD, port=FACTORIO_RCON_PORT) as mcr:
        ...
        if "Online players" in players_resp:
            ...
            for line in player_lines:
                if line.strip():
                    ...
                    playtime = parts[1].split(')')[0].strip() if len(parts) > 1 else "Unknown"
...
async def check_server_status():
    ...
    if status['status'] == 'online':
        ...
        embed.add_field(name="Players Online", value=f"{status['player_count']}", inline=True)
        embed.add_field(name="Game Version", value=status['game_version'], inline=True)
        embed.add_field(name="Server Lifetime", value=status['server_lifetime'], inline=False)
